###Week3 case 2 - Forecasting Elantra sales  SOLUTION ##########
...
nrow(train)
#Solution: There are 36 observations.
...
lrElantra=lm(ElantraSales~Unemployment+CPI_all+CPI_energy+Queries,data=train)
summary(lrElantra)
#Solution: The multiple R-squared is 0.4282. The adjusted R-squared is 0.3544


#3. How many variables are significant, or have levels that are significant? Use 0.10 as your p-value cutoff.
# Solution: 0 variables are significant
...
lrElantraSeasonal=lm(ElantraSales~Unemployment+Queries+CPI_energy+CPI_all+Month,data=train)
summary(lrElantraSeasonal) 
#Solution: The multiple R-squared is 0.4344. The adjusted R-squared is 0.3402
...
#Solution: Sales incremented in 110.69*(3-1)=221.38
...
#Solution: Sales incremented in 110.69*(5-1)=442.76  
...
lrElantraSeasonal=lm(ElantraSales~Unemployment+Queries+CPI_energy+CPI_all+Month,data=train)
summary(lrElantraSeasonal)  
## Solution: multiple R2 increased to 81.93%, adjusted R2 to 68.37%



#10. Which variables are significant, or have levels that are significant? Use 0.10 as your p-value cutoff. 
#Solution: Unemployment, CPI_energy, CPI_all, Month
...
cor(train)
##Solution: Unemployment, Queries, CPI_all
...
lrElantraSeasonal=lm(ElantraSales~Unemployment+Queries+CPI_energy+CPI_all+Month,data=train)
summary(lrElantraSeasonal) 
lrElantraSeasonal2=lm(ElantraSales~Unemployment+CPI_energy+CPI_all+Month,data=train)
summary(lrElantraSeasonal2)
#Solution: We only removed the variable Queries



#13. Use the model from Problem 12, make predictions on the test set. What is the sum of squared errors of the model
#    on the test set?
predictionTest=predict(lrElantraSeasonal2,newdata=test)
SSE=sum((test$ElantraSales-predictionTest)^2)
SSE
#Solution: 190757747
...
max(abs(predictionTest-test$ElantraSales))
##Solution = 7491.488


#16. In which period (Month,Year) do we make the largest absolute error in our prediction?
which.max(abs(predictionTest-test$ElantraSales))
head(test)
#Solution: This function returns 5 which is the row number in test corresponding to the period for 
#  which we make the largest absolute error. Then use head(test) and see row#5 (with value 14). 
#  Then we observe 03/2013
